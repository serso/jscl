...
public final class JsclInteger extends Numeric {

	@NotNull
	private final RawNumber content;

	public JsclInteger(RawNumber content) {
	...
	@NotNull
	public JsclInteger add(@NotNull JsclInteger that) {
		return new JsclInteger(content.add(that.content));
	}
	...
	@NotNull
	public JsclInteger subtract(@NotNull JsclInteger that) {
	...
	@NotNull
	public JsclInteger multiply(@NotNull JsclInteger integer) {
...
*/
	...
	public Numeric gcd(@NotNull Numeric generic) {
		if (generic instanceof JsclInteger) {
	...
	public Numeric ln() {
		if (signum() >= 0) {
			return new Real(getMathContext(), Math.log(content.doubleValue()));
		} else {
			return new Complex(getMathContext(), Math.log(-content.doubleValue()), Math.PI);
		}
	}
	...
	public Numeric lg() {
		if (signum() >= 0) {
			return new Real(getMathContext(), Math.log10(content.doubleValue()));
		} else {
			return new Complex(getMathContext(), Math.log10(-content.doubleValue()), Math.PI);
		}
	}

	@NotNull
	public Numeric exp() {
		return new Real(getMathContext(), Math.exp(content.doubleValue()));
	}
