...
public class VectorParser implements Parser {
    ...
    public Object parse(@NotNull String expression, @NotNull MutableInt position, Generic previousSumElement) throws ParseException {
        ...
        try {
            Generic a=(Generic)ExpressionParser.parser.parse(expression, position, previousSumElement);
            l.add(a);
        ...
        while(true) {
            try {
                Generic a=(Generic)CommaAndExpression.parser.parse(expression, position, previousSumElement);
                l.add(a);
            } catch (ParseException e) {
                break;
            }
